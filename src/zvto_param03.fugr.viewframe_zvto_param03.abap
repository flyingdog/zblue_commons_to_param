*---------------------------------------------------------------------*
*    program for:   VIEWFRAME_ZVTO_PARAM03
*---------------------------------------------------------------------*
FUNCTION VIEWFRAME_ZVTO_PARAM03        .
...
*----------------------------------------------------------------------*
CALL FUNCTION 'VIEWPROC_ZVTO_PARAM03'
         EXPORTING
              FCODE          = READ
...
EXTRACT = ZVTO_PARAM03_EXTRACT
TOTAL = ZVTO_PARAM03_TOTAL
              X_HEADER       = X_HEADER
              X_NAMTAB       = X_NAMTAB
...
CALL FUNCTION 'VIEWPROC_ZVTO_PARAM03'
             ...
             IMPORTING
                  UCOMM           = FUNCTION
UPDATE_REQUIRED = STATUS_ZVTO_PARAM03-UPD_FLAG
             TABLES
                  EXCL_CUA_FUNCT  = EXCL_CUA_FUNCT
EXTRACT = ZVTO_PARAM03_EXTRACT
TOTAL = ZVTO_PARAM03_TOTAL
...
*----------------------------------------------------------------------*
      ...
      IF ( FUNCTION EQ SWITCH_TO_SHOW_MODE OR
           ...
           FUNCTION EQ END ) AND
STATUS_ZVTO_PARAM03-UPD_FLAG NE SPACE.
        PERFORM BEENDEN.
        CASE SY-SUBRC.
          WHEN 0.
CALL FUNCTION 'VIEWPROC_ZVTO_PARAM03'
...
TOTAL = ZVTO_PARAM03_TOTAL
            ...
            CASE SY-SUBRC.
              WHEN 0.
IF STATUS_ZVTO_PARAM03-UPD_FLAG EQ SPACE. EXIT. ENDIF.
...
IF STATUS_ZVTO_PARAM03-UPD_FLAG NE SPACE.
